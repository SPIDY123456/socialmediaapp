...
const Home = () => {
    ...
    useEffect(() => {
        socket.on('sendGroupMessage', (group) => {
            console.log("Messages received", group);
            setStories(prevGroup => [group, ...prevGroup]); // Add new story to the front
        });

        return () => {
            socket.off('sendGroupMessage');
        };
    }, []);


    useEffect(() => {
        socket.on('liveStreamNotifications', (stream) => {
            console.log("recievedStreamNotifications", stream);
            setStories(prevStream => [stream, ...prevStream]); 
        });

        return () => {
            socket.off('liveStreamNotifications');
        };
    }, []);
